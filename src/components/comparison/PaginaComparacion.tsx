...
import { Loader, Sparkles, PlusCircle, Car as IconoAuto } from 'lucide-react';
import { summarizeCarComparison } from '@/ai/flows/summarize-car-comparison';
import FormularioCapturaLeads from '../shared/FormularioCapturaLeads';
...
export default function PaginaComparacion({ autos, todosLosAutos }: PaginaComparacionProps) {
  ...
  const manejarSeleccionAuto = (position: 'car1' | 'car2', carId: string) => {
    const idsActuales = autos.map(c => c.id);
    let nuevosIds: string[];

    if (position === 'car1') {
        nuevosIds = [carId, idsActuales[1]].filter(Boolean);
    } else {
        nuevosIds = [idsActuales[0], carId].filter(Boolean);
    }
    router.push(`/compare?ids=${nuevosIds.join(',')}`);
  };

  const manejarResumenIA = () => {
    if (!auto1 || !auto2) return;
    const caracteristicasAuto1 = JSON.stringify(auto1);
    const caracteristicasAuto2 = JSON.stringify(auto2);
    const necesidadesUsuario = "Quiero un auto equilibrado. Considera el precio, el rendimiento y las características.";

    startTransition(async () => {
        setResumen(null);
        try {
            const result = await summarizeCarComparison({ car1Features: caracteristicasAuto1, car2Features: caracteristicasAuto2, userNeeds: necesidadesUsuario });
            setResumen(result);
        } catch (error) {
  ...
  const formatarValor = (key: string, car?: Auto) => {
    if (!car) return '-';
    ...
    switch (key) {
      ...
      case 'transmission': 
      case 'color':
        const claveTraduccion = key as keyof typeof traducciones;
        if (claveTraduccion in traducciones) {
            const claveValor = value as keyof typeof traducciones[typeof claveTraduccion];
            if (claveValor in traducciones[claveTraduccion]) {
                return traducciones[claveTraduccion][claveValor];
            }
        }
        return value;
  ...
  const SelectorAuto = ({ autoSeleccionado, alSeleccionar, posicion, otroAutoId }: { autoSeleccionado?: Auto, alSeleccionar: (pos: 'car1' | 'car2', carId: string) => void, posicion: 'car1' | 'car2', otroAutoId?: string }) => {
    const autosDisponibles = todosLosAutos.filter(c => c.id !== otroAutoId);
    
    if (autoSeleccionado) {
      return (
        <Card className="overflow-hidden md:col-span-1 w-full">
            <div className="aspect-video relative">
                {autoSeleccionado.imageUrl ? (
                    <Image src={autoSeleccionado.imageUrl} alt={`${autoSeleccionado.brand} ${autoSeleccionado.model}`} fill className="object-cover" />
                ) : (
            ...
            <CardHeader>
                <CardTitle>{autoSeleccionado.brand} {autoSeleccionado.model}</CardTitle>
            </CardHeader>
            <CardContent>
                <p className="text-2xl font-bold">{formatarValor('price', autoSeleccionado)}</p>
                <p className="text-sm text-muted-foreground">{autoSeleccionado.year}</p>
            </CardContent>
        </Card>
      )
    }
    ...
    return (
        <Card className="w-full h-full min-h-[300px] flex flex-col items-center justify-center border-dashed p-4">
            ...
            <Select onValueChange={(carId) => alSeleccionar(posicion, carId)}>
                <SelectTrigger className="w-full max-w-xs">
                    <SelectValue placeholder="Seleccionar un auto" />
                </SelectTrigger>
                <SelectContent>
                    {autosDisponibles.map(car => (
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <SelectorAuto autoSeleccionado={auto1} alSeleccionar={manejarSeleccionAuto} posicion="car1" otroAutoId={auto2?.id} />
        <SelectorAuto autoSeleccionado={auto2} alSeleccionar={manejarSeleccionAuto} posicion="car2" otroAutoId={auto1?.id} />
      </div>
          ...
          {caracteristicas.map(feature => (
            <div key={feature.key}>
              <div className="grid grid-cols-3 items-center gap-4">
                ...
                <div className="text-center col-span-1">{formatarValor(feature.key, auto1)}</div>
                <div className="text-center col-span-1">{formatarValor(feature.key, auto2)}</div>
              </div>
      ...
      <div className="text-center">
          <Button size="lg" onClick={manejarResumenIA} disabled={isPending || !auto1 || !auto2}>
      ...
      {(auto1 || auto2) && (
        <Card>
          ...
          <CardContent>
              ...
              <FormularioCapturaLeads autosInteres={[auto1, auto2].filter(Boolean).map(c => `${c?.brand} ${c?.model}`).join(', ')} />
          </CardContent>
        </Card>
      )}
